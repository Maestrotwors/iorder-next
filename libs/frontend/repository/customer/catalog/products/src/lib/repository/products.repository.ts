...
export class CatalogProductsRepository {
  private productsStore = createStore({ name: 'catalogProducts' }, withEntities<Product>());
  ...
  products$ = this.productsStore.pipe(selectAllEntities(), joinRequestResult(['catalogProducts']));
  productsCount$ = this.productsInfoStore.pipe(select(store => store.totalCount));
